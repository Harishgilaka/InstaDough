using InstaDough.Core.BusinessManager.Exceptions;
using InstaDough.Core.BusinessManager.Interface;

namespace InstaDough.Core.BusinessManager
{
    public class Dispatcher : IDispatcher
    {
        ...
        public void Execute<TCommand>(TCommand command) where TCommand : ICommand
        {
            ...
            {
                throw (new CommandHandlerNotFoundException(String.Format("Dispatcher did not find registered handler for type {0}", typeof(TCommand).ToString()), ex));
            }
        }
        ...
        public TResult Execute<TQuery, TResult>(TQuery query) where TQuery : IQuery<TResult>
        {
            ...
            {
                throw (new QueryHandlerNotFoundException(String.Format("Dispatcher did not find registered handler for type {0}", typeof(TQuery).ToString()), ex));
            }
